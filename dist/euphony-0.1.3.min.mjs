...
window.Euphony = function () {
  function euphony() {
    ...
    for (let i = 0; i < 16; i++) this.outBuffer[i] = this.crossfadeStaticBuffer(this.makeStaticFrequency(this.ZEROPOINT + i * this.SPAN));
  ...
  euphony.prototype = {
    setCode: function setCode(data) {
      ...
      for (let i = 0; i < data.length; i++) code += data.charCodeAt(i).toString(16);
      ...
      if (T.isAudioWorkletAvailable) {
        ...
        */
        ...
        audioWorklet.addModule('./euphony-processor.mjs').then(() => {
          ...
          euphonyWorkletNode.connect(T.context.destination); //source.connect(euphonyWorkletNode).connect(T.context.destination);
        ...
        T.scriptProcessor.onaudioprocess = function (e) {
          ...
          for (let i = 0; i < outputBuf.length; i++) outputBuf[i] = outputBuf2[i] = T.playBuffer[T.playBufferIdx][i];
    ...
    makeStaticFrequency: function makeStaticFrequency(freq) {
      let T = this;
      let buffer = new Array();

      for (let i = 0; i < T.BUFFERSIZE; i++) buffer[i] = Math.sin(T.PI2 * freq * (i / T.SAMPLERATE));

      return buffer;
    },
    crossfadeStaticBuffer: function crossfadeStaticBuffer(buffer) {
      let T = this;
      var mini_window,
    ...
    mixRawData: function mixRawData() {
      let T = this;
    ...
    maximizeVolume: function maximizeVolume() {
      let T = this; //SCAN FOR VOLUME UP

      var max = 0;

      for (let i = 0; i < T.BUFFERSIZE; i++) {
        if (max < Math.abs(outBuffer[i])) max = T.outBuffer[i];
      }

      if (32767 == max) T.outBuffer[i] *= 32767 / max;
    },
    makeChecksum: function makeChecksum(code) {
      var sum = 0;
    ...
    makeParallelParity: function makeParallelParity(code) {
      let parity1 = 0,
          parity2 = 0,
          parity3 = 0,
          parity4 = 0,
