...
export var Euphony = (function () {
  ...
  euphony.prototype = {
    ...
    setCode: function (data) {
      ...
      let dataCode = ''
      for (let i = 0; i < data.length; i++) { dataCode += data.charCodeAt(i).toString(16) }
      code += dataCode
    ...
    makeZeroSource: function () {
      ...
      for (let i = 0; i < T.BUFFERSIZE; i++) { buffer[i] = 0 }
    ...
    makeStaticFrequencyBySAB: function (freq, buffer) {
      ...
      for (let i = 0; i < T.BUFFERSIZE; i++) { pBuffer[i] = Math.sin(T.PI2 * freq * (i / T.SAMPLERATE)) }
      return buffer
    },
    ...
    crossfadeStaticBuffer: function (buffer, cf_type = 3) {
      ...
      let pBuffer = null
      if (buffer.constructor === Float32Array) { pBuffer = buffer } else { pBuffer = new Float32Array(buffer) }
    ...
    maximizeVolume: function () {
      ...
      for (i = 0; i < T.BUFFERSIZE; i++) {
        if (max < Math.abs(T.outBuffer[i])) { max = T.outBuffer[i] }
      }

      if (max === 32767) { T.outBuffer[i] *= 32767 / max }
    },
